...
@router.get(”/google”)
def login_google(redirect_to: str = “/”):
params = {
...
@router.get(”/callback”)
async def google_callback(code: str, response: Response, state: str = “/”, db: Session = Depends(get_db)):
...
redirect = RedirectResponse(url=f"{FRONTEND_URL}{state}")
...
create_session_cookie(redirect, new_user.user_id)
return redirect
```
